...
//   colorWipe(strip.Color(0,127,0), 20);      // blue
...
//   wave(strip.Color(127,0,0), 4, 20);        // candy cane
  ...
  // Serial.print("\t Rand Color: ");
  // Serial.print(randColor);
  // Serial.println();
  revealAndPlayEffect(randColor);
  // colorEffects(BlueColor);
  // colorEffects(MagentaColor);
  // colorEffects(GreenColor);
  // colorEffects(YellowColor);
  // colorEffects(RedColor);
  // colorEffects(CyanColor);
...
void revealAndPlayEffect(uint32_t color){
  ...
  // Serial.print("\t Rainbow: ");
    ...
    randomReveal(delayMsec);
    rainbowCycle(1, delayMsec);
  }
  if(effect == 2){
  // Serial.print("\t Color Scanner: ");
  byte r = getRGB(color, 'r');
  byte g = getRGB(color, 'g');
  byte b = getRGB(color, 'b');
  scanner(r,g,b, delayMsec);
...
void randomReveal(uint8_t wait){
  ...
  //   Serial.print("\t Rand Reveal: ");
  // Serial.print(randRevealIndex);
  // Serial.println();
...
void colorEffects(uint32_t color){
  ...
  ///randomise a set of effects+color - wave, rainbow, scanner
  /// within each eeffect, randomise a set of reveals: wipe, dither
}
...
void basicEffects(){
  ...
  // wave(strip.Color(128,100,0), 5, 1);        // icy
  // colorWipe(strip.Color(0,0,0), 20);        // black
  rainbowWipe(delayMsec);
  // colorWipe(strip.Color(127,127,0), 20);        // magenta
...
void rainbowCycle(int repeat, uint8_t wait) {
  uint16_t i, j;
  for (j=0; j < 384 * repeat; j++) {     // 5 cycles of all 384 colors in the wheel
